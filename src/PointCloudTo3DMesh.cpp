...
void PointCloudTo3dMesh::init() {
  ...
  ros_nh.getParam("/PointCloudTo3dMesh", params);
  ...
  _pointCloudPub.reset(new ros::Publisher(ros_nh.advertise<sensor_msgs::PointCloud2>("PointCloud",5)));
...
void PointCloudTo3dMesh::get3DMeshFromPointCloud() {
  ...
  rosCloudMsg.header = _rgbd_sub->get_depth().header;
